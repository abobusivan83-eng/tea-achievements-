...
function RatingListInner(props: {
  ...
  const tooltipDesktop = useMediaQuery("(min-width: 1024px)");
      ...
      {props.rows.map((row, idx) => {
        ...
        const rowCard = (
          <motion.button
            ...
            className={clsx(
              "group relative w-full overflow-hidden rounded-[24px] border px-4 py-4 text-left shadow-[0_12px_30px_rgba(0,0,0,0.24)] transition-all duration-300 hover:border-white/15 hover:shadow-[0_18px_36px_rgba(0,0,0,0.32)]",
              accentClass,
            )}
        ...
        return (
          <div key={row.id} className="min-w-0">
            {tooltipDesktop ? <Tooltip content={tooltipContent}>{rowCard}</Tooltip> : rowCard}
          </div>
        );
      })}
    </div>
